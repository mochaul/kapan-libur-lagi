import { Holiday } from '@/types';

export const fallbackHolidays: Holiday[] = [
  { name: "Hari Raya Natal", date: "2025-12-25" },
  { name: "Tahun Baru Masehi", date: "2026-01-01" },
  { name: "Isra Mi'raj", date: "2026-01-16" },
  { name: "Tahun Baru Imlek", date: "2026-02-17" },
  { name: "Hari Suci Nyepi", date: "2026-03-19" },
  { name: "Idul Fitri (Hari 1)", date: "2026-03-21" },
  { name: "Idul Fitri (Hari 2)", date: "2026-03-22" },
  { name: "Wafat Yesus Kristus", date: "2026-04-03" },
  { name: "Hari Buruh Internasional", date: "2026-05-01" },
  { name: "Kenaikan Yesus Kristus", date: "2026-05-14" },
  { name: "Idul Adha", date: "2026-05-27" },
  { name: "Hari Raya Waisak", date: "2026-05-31" },
  { name: "Hari Lahir Pancasila", date: "2026-06-01" },
  { name: "Tahun Baru Islam", date: "2026-06-16" },
  { name: "Hari Kemerdekaan", date: "2026-08-17" },
  { name: "Maulid Nabi Muhammad SAW", date: "2026-08-25" },
  { name: "Hari Raya Natal", date: "2026-12-25" },
];

